...
public class Friends {
    ...
    private static class Graph {
        ...
        public Student getStudentByName(String name) {
            return doseStudentExist(name) ? students[studentIndexTable.get(name)] : null;
        }
    ...
    private static class Student {
        ...
        public String toString() {
            return String.format("(%s%s)", name, isStudent ? ", " + school : "");
        }

        @Override
        public boolean equals(Object obj) {
            return obj instanceof Student && name.equals(((Student) obj).getName()) && school.equals(((Student) obj).getSchool());
        }
    }
    ...
    private static class FriendshipSP {
        ...
        public FriendshipSP(Graph graph, String fromName) {
            ...
            for(Student s : graph.getStudents()) { distTo.put(s, Integer.MAX_VALUE); }
            distTo.put(from, 0);
        ...
        public Stack<Student> friendshipChain(Student to) {
            ...
            if (!hasFriendship(to)) return null;
            for (Student s = to; friendshipTo.get(s) != null; s = friendshipTo.get(s)) {
    ...
    private static void printMenu() {
        ...
        menu.forEach((k, v) -> System.out.format("%s. %s%n", k, v));
        System.out.println();
    }
    ...
    private static String introFriend(String nameFrom, String nameTo) {
        if (graph.lookupStudentIndex(nameFrom) == -1) {
            return String.format("Graph doesn't contain the student with name: %s", nameFrom);
        }

        if (graph.lookupStudentIndex(nameTo) == -1) {
            return String.format("Graph doesn't contain the student with name: %s", nameTo);
        }
        ...
        return String.format("No relationship between %s and %s", nameFrom, nameTo);
    }
    ...
    private static void printCliques(String school) {
        ...
        for (int i = 0; i < graph.getNumOfStudent(); i++) {
            ...
            if (student.isStudent() && student.getSchool().equals(school) && !printedStudent.contains(student)) {
                queue.clear();
                ...
                while (!queue.isEmpty()) {
                    ...
                    if (!visitedStudent.contains(currentStudent)) {
                        currentStudent.getFriends().forEach(e -> {
                            if (e.isStudent() && e.getSchool().equals(school)) queue.add(e);
                        });
                ...
                if (visitedStudent.size() != 0) {
                    ...
                    visitedStudent.forEach(e -> System.out.printf("%s|%s%n", e.getName(), e.isStudent ? "y|" + e.getSchool() : "n"));
                    visitedStudent.forEach(e -> {
                        e.getFriends().forEach(f -> {
                            if (!printedStudent.contains(f) && f.isStudent && f.getSchool().equals(school))
                                System.out.printf("%s|%s%n", e.getName(), f.getName());
                        });
        ...
        if (cliqueCount == 0) System.out.println("There is not clique in selected school");
        System.out.println();
    }
    ...
    private static void DFS(int[] dfsnum, int[] back, boolean isVisited[], int countDfsnum, int studentIndex) {
        if (isVisited[studentIndex]) return;
        ...
        for (Student s : student.getFriends()) {
            ...
            if (isVisited[nextStudentIndex]) {
                back[studentIndex] = Math.min(back[studentIndex], dfsnum[nextStudentIndex]);
            } else {
                DFS(dfsnum, back, isVisited, countDfsnum, nextStudentIndex);
                if (dfsnum[studentIndex] > back[nextStudentIndex]) {
                    back[studentIndex] = Math.min(back[studentIndex], back[nextStudentIndex]);
                } else {
                    if (studentIndex == 0) {
    ...
    public static void main(String[] args) {
        ...
        Scanner in = new Scanner(System.in);
        String graphFilePath = "Rutgers/" + in.nextLine();
        System.out.println();
